...
const ProductDetail = () => {
  ...
  const hasReviewed = product.reviews?.some((review) => review.user === user?.id);
  ...
  return (
    <section className="stack">
          ...
          <div className="buy-row">
            <div className="stepper">
              ...
              <input
                ...
                max={product.stock}
                value={qty}
                onChange={(event) => setQty(Math.max(1, Math.min(Number(event.target.value), product.stock)))}
              />
              <button type="button" onClick={() => setQty(Math.min(product.stock, qty + 1))}>
                <Plus size={16} />
      ...
      <section className="reviews-section">
        ...
        {user && !hasReviewed && <ReviewForm onSubmit={submitReview} submitting={reviewing} />}
        {!user && (
          <p className="muted">
          ...
          <div className="review-list">
            {product.reviews.map((review) => (
              <article className="review-card" key={review._id}>
                ...
                <p>{review.comment}</p>
                {review.user === user?.id && (
                  <button className="text-button danger-text" type="button" onClick={deleteReview}>
